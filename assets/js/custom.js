...
$(document).ready(function () {
  // Toggle menu
  $('.as_toggle').on('click', function () {
    $('.as_menu_wrapper').toggleClass('menu_open');
    $('body').toggleClass('menu_is_open');

    // Log for debugging
    console.log(
      'Toggle clicked, menu_open class:',
      $('.as_menu_wrapper').hasClass('menu_open')
    );
    console.log('Menu links:', $('.as_menu_wrapper .as_menu ul li').length);
  });

  // Close menu
  $('.as_menu_close, .as_body_overlay').on('click', function () {
    $('.as_menu_wrapper').removeClass('menu_open');
  ...
  // Handle submenu toggling if needed
  $('.mobile_menu > ul > li.has_submenu > a').on('click', function (e) {
    if ($(window).width() <= 991) {
      e.preventDefault();
      $(this).next('.as_submenu').slideToggle();
    }
  });
});
